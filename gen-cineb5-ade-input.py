...
main_text = (
    ...
    "print('Total number of CI-NEB iters =', cineb.images[1].iteration*5)\n"
)
...
for i in range(0, 8):
    ...
    # create folder
    pathlib.Path(f"./ci_neb_5/reaction-{i}/").mkdir(parents=True, exist_ok=True)
    # copy xyz files
    shutil.copyfile(f"./final_aligned_orca_opt/reaction-{i}/reaction-{i}-rct-orca.xyz",
                    f"./ci_neb_5/reaction-{i}/reaction-{i}-rct-orca.xyz")
    shutil.copyfile(f"./final_aligned_orca_opt/reaction-{i}/reaction-{i}-prod-orca.xyz",
                    f"./ci_neb_5/reaction-{i}/reaction-{i}-prod-orca.xyz")
    # write input files
    mid_txt = (
    ...
    with open(f"ci_neb_5/reaction-{i}/reaction-{i}-ci-neb-5.py", "w") as fh:
        fh.write(
            begin_text + mid_txt + main_text
        )
    
    pathlib.Path(f"./ci_neb_5/reaction-{i}/ts-opt/").mkdir(exist_ok=True)
    ts_inpfilebase = f"reaction-{i}-cineb5_optts"
    with open(f"./ci_neb_5/reaction-{i}/ts-opt/"+ts_inpfilebase+".inp", "w") as fh2:
        text = ts_template.replace("template.xyz", "../cineb-5-peak.xyz")
